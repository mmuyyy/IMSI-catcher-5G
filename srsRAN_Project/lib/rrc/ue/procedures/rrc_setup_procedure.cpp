...
void rrc_setup_procedure::operator()(coro_context<async_task<void>>& ctx)
{
  ...
  // IMSI catcher modification: Allow connection even if PLMN is not supported
  ...
  // IMSI catcher modification: Send identity request after RRC connection
  rrc_ue.send_identity_request();

  // send_initial_ue_msg(); // IMSI catcher modification: Skip sending to core network
